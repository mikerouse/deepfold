...
**Both.** Schema lives in `apps/api/app/models.py` and Alembic. Persist every journalist decision through `POST /drafts/{id}/decisions`. Pipeline stages are Pitch → Drafting → Checking → Publication → Social. Pitch items are abstracts; **Go** enqueues `draft_article` (and image/localise) jobs and leaves them queued for Grok Bot unless `DEMO_INSTANT_FULFILL=true`. Grok Bot claims jobs and POSTs results; `DEMO_GROK_WORKER` is a labelled stub for localhost only. Do not add vendor LLM keys to the app. Never auto-publish `single_source`, `caution`, or `defamation_sensitive` copy. Honour `KILL_SWITCH` and `APPROVE_AND_PUBLISH_ENABLED` (off by default). Title targeting is search + packages, not a flat checklist of the registry.
